...
AFRAME.registerComponent('number-lock', {
    ...
    init: function() {      
        ...
        //connect to web sockets so we can sync the combination lock between users
        // CONTEXT_AF.socket     = null;
        // CONTEXT_AF.connected  = false;
        // CONTEXT_AF.combinationEventName = "combinationLock_event";
        ...
        //     //listen for when others switch combination slot
        //     CONTEXT_AF.socket.on(CONTEXT_AF.combinationEventName, function(data) {
        //         //console.log(data.combinationEventNam)
        //         //console.log(data.rotatedSlot);
        //         //data.rotatedSlot.emit('rotate-slot');
        //         console.log("!");
        //     });
        ...
        //             CONTEXT_AF.socket.emit(CIRCLES.EVENTS.SEND_DATA_SYNC, {rotatedSlot:'test', room:CIRCLES.getCirclesGroupName(), world:CIRCLES.getCirclesWorldName()});
        //         }
        //     });
        ...
        CONTEXT_AF.objWidth = objScale.split(';')[1].split(':')[1];
        CONTEXT_AF.objHeight = objScale.split(';')[2].split(':')[1];
        CONTEXT_AF.objDepth = objScale.split(';')[2].split(':')[1];
        CONTEXT_AF.slotWidth = objWidth / CONTEXT_AF.data.nums;
        ...
        for (let i = 0; i < CONTEXT_AF.data.nums; i++) {
            ...
            numSlot.addEventListener('click', function () {
                ...
                //CONTEXT_AF.socket.emit(CONTEXT_AF.combinationEventName, {rotatedSlot:CONTEXT_AF.numSlots[i], room:CIRCLES.getCirclesGroupName(), world:CIRCLES.getCirclesWorldName()});
            });
